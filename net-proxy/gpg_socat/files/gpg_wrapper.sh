...
configure() {
  ...
  [ -z "${exe:-}" ] && readonly exe="$(command -v gpg)"
  [ -z "${log_file:-}" ] && readonly log_file="${TMP:-/tmp}/gpg_wrapper.log"
  [ -z "${mtu:-}" ] && readonly mtu='1500'
  [ -z "${scheme:-}" ] && readonly scheme='socks5h'
  [ -z "${socket_dir:-}" ] && readonly socket_dir='/var/run/qrtunnels/gpg'
  ...
  [ -z "${env_keep:-}" ] && readonly env_keep="_ HOME LANG PWD PATH SHELL TERM TMP USER"
}
